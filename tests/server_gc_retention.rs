...
#[test]
fn server_gc_prunes_unpinned_objects_and_metadata() -> Result<()> {
    ...
    client
        .post(format!(
            ...
            server.base_url, bundle1_id
        ))
        .header(reqwest::header::AUTHORIZATION, &auth)
        .send()
        .context("pin bundle1")?
        .error_for_status()
        .context("pin bundle1 status")?;

    // Run server GC: keep pinned bundle + its provenance, prune everything else.
    ...
    // Only publication1 should remain.
    let pubs: Vec<serde_json::Value> = client
        .get(format!("{}/repos/test/publications", server.base_url))
        .header(reqwest::header::AUTHORIZATION, &auth)
        .send()
    ...
    assert_eq!(
        pubs[0].get("snap_id").and_then(|v| v.as_str()),
        Some(snap1.id.as_str())
    );

    // Only bundle1 should remain.
    let bundles: Vec<serde_json::Value> = client
        .get(format!("{}/repos/test/bundles", server.base_url))
        .header(reqwest::header::AUTHORIZATION, &auth)
        .send()
    ...
    assert_eq!(
        bundles[0].get("id").and_then(|v| v.as_str()),
        Some(bundle1_id.as_str())
    );
