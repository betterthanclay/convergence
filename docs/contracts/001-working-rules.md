...
4. the active Northstar task (`gNN.NNN`)
5. `docs/specs/README.md`
6. the active spec and current ready task
...
Implementation work should only proceed when a bounded ready Northstar task exists.

If there is no ready task, the lane is in planning. Do not infer execution from
...
That `Next Task` should normally point at the current ready task. If it does
...
A completed task must never remain advertised as the current ready task.
...
Treat roadmap generations as substantial sequencing eras, not tiny buckets. In a long-running repo, expect roughly 20 to 40 task files in one generation before rollover is even worth discussing.
...
Apply these rules to the closing `g02` generation. No ready task;
